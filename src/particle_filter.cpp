...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
  ...
  for(int i = 0; i < observations.size(); i++)
  {
    ...
    {
      double dst = dist(landmark.x, landmark.y, observations[i].x, observations[i].y);
      if (dst < min_distance)
      {
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for(int i = 0; i < num_particles; i++)
  {
    ...
    for (Map::single_landmark_s singleLmk : map_landmarks.landmark_list)
    {
      ...
      if(dist(singleLmk.x_f, singleLmk.y_f, particles[i].x, particles[i].y) <= sensor_range)
      {
    ...
    {
      //calculate exponent
      vector<LandmarkObs>::iterator it =
       find_if(predictedLmks.begin(), predictedLmks.end(), [&](const LandmarkObs & o) {
